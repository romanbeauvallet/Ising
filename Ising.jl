...
"""
function tebd_sweep(mps, Beta, J, h, Dmax, rejected_weight, cutoff)
    p = length(mps)
    ...
    for i in 1:2:p-3 #n est even
        #@show i, size(right[i])
        mpsnew[i], tempmps[i+1] = tebd2(right[i], right[i+1], gate, "right", Dmax, cutoff, rejected_weight)
        #@show size(mpsnew[i])
        #changement de jauge
    ...
    mpsnew[p-1], mpsnew[p] = tebd2(right[p-1], right[p], gate, "right", Dmax, cutoff, rejected_weight)
    ...
    for i in p-1:-2:3
        ...
        tempmps[i-1], mpsnew[i] = tebd2(mpsnew[i-1], mpsnew[i], gate, "left", Dmax, cutoff, rejected_weight)
        #changement de jauge
...
"""
function ising2D(N::Int, d, D, J::Float64, h::Float64, Beta::Float64, n, cutoff, Dmax, side, rejected_weight)
    # Create the MPS for the 2D Ising model
    mps = MPS(N, d, D)
    # Sweeps
    for i in 1:n
        mps = ising2Dunit(mps, J, h, Beta, n, cutoff, Dmax, side, rejected_weight)
    end
    return mps
end

function ising2D2(N, D0, d, J, h, Beta, n, Dmax, rejected_weight, cutoff)
    mps = init_random_mps(N, d, D0)
    for i in 1:n
        mps = tebd_sweep(mps, Beta, J, h, Dmax, rejected_weight, cutoff)
    end
    return mps
end
...
MPSlist = map(β -> ising2D2(N, D0, d, J, h, β, n, Dmax, rejected_weight, cutoff), Betalist)
Elist = map(mps -> energyIsing(mps, J, site_measure), MPSlist)
Eexact = ExactEnergy.(Betalist)

#display(current())
mps = init_random_mps(N, d, D0)
e1 = energyIsing(deepcopy(mps), J, site_measure)
e2 = energyIsing(deepcopy(mps), J, site_measure)
